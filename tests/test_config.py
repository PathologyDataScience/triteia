...
def test_response_cache(data, triton):
    ...
    updated = model.get_config()
    assert (
        updated["responseCache"] == {"enable": False} or updated["responseCache"] == {}
    )
...
def test_add_mixed_precision(data, triton):
    ...
    ampxla_config = TensorflowConfig(
        MODEL, max_batch_size=64, optimization=optimization
    )
    ...
    updated = model.get_config()
    assert updated["optimization"]["executionAccelerators"] == {
    ...
    model.load(config=config.json())
    updated = model.get_config()
    assert "executionAccelerators" not in updated["optimization"]
...
def test_add_trt(data, triton):
    ...
    def test_fp(precision_mode):
        ...
        assert (
            "executionAccelerators" in updated
            and "gpuExecutionAccelerator" in updated["executionAccelerators"]
        )
        updated = updated["executionAccelerators"]["gpuExecutionAccelerator"][0]
        assert (
            "name" in updated
            and "parameters" in updated
            and "precision_mode" in updated["parameters"]
        )
