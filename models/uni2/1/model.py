"""Triton Python backend model for UNI2-h on Quest.
...
# Architecture parameters from the UNI2-h model card, used only if the
# downloaded config.json does not declare them. Every value must match the
# published model exactly: a mismatch in these does not raise, it produces
# plausible and incorrect embeddings.
UNI2_FALLBACK = {
...
class TritonPythonModel:
    ...
    def auto_complete_config(model_config):
        """Returns a minimal model configuration for the uni2 model.
    ...
    def initialize(self, args):
        """Build UNI2-h from weights staged in the user's weights directory."""
        ...
        # mlp_layer and act_layer are not JSON-serialisable, so they are always
        # supplied here rather than read from the repository config.
        self.model, pretrained_cfg = _timm_from_local(
            weights_dir,
            fallback_args=UNI2_FALLBACK,
            mlp_layer=timm.layers.SwiGLUPacked,
            act_layer=torch.nn.SiLU,
        )
