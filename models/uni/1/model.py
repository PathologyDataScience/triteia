"""Triton Python backend model for UNI on Quest.
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        """Build UNI from weights staged in the user's weights directory."""
        ...
        # init_values / dynamic_img_size are passed explicitly upstream, so keep
        # them as overrides on top of whatever the repo config declares.
        self.model, pretrained_cfg = _timm_from_local(
            weights_dir, init_values=1e-5, dynamic_img_size=False
        )

        self.device = torch.device(f"cuda:{self.gpu_id}")
        self.model = self.model.to(self.device)
        self.mean = (
            torch.tensor(pretrained_cfg["mean"]).view(1, 3, 1, 1).to(self.device)
        )
        self.std = (
            torch.tensor(pretrained_cfg["std"]).view(1, 3, 1, 1).to(self.device)
        )
