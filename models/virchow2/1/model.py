"""Triton Python backend model for Virchow2 on Quest.

See https://huggingface.co/paige-ai/Virchow2

The embedding is 2560 = 2 x 1280: the class token concatenated with the mean of
the patch tokens. Token 0 is the class token, tokens 1-4 are register tokens, so patch
tokens start at index 5. The register tokens must therefore be present in the
rebuilt architecture: they come from model_args in the repo config.
"""
...
class TritonPythonModel:
    ...
    def initialize(self, args):
        """Build Virchow2 from weights staged in the user's weights directory."""
