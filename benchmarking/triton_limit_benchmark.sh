...
cmd="python ./benchmarking/triton_benchmark.py --model-name ${modelname} --output ${output_dir} --wsi-path ${wsi_path} --batch-size 256 --url localhost:7985 --metrics-endpoint localhost:7986/metrics"

if [[ "${modelname}" == "resnet50" || "${modelname}" == "prov-gigapath" ]]; then
  cmd="${cmd} --nchw"
fi
...
do
  ...
  do
    ...
    tb_name="triton${modelname}gpu${gpu}limit${limit}"
    if [[ "${inference_only}" == "1" || "${inference_only}" == "True" ]]; then
    ...
    clear_cache
    eval "$cmd --limit $limit --tensorboard-name ${tb_name} --gpus ${gpu_string}"
    set +xe
  done
